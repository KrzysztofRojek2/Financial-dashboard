...
const ExpensesChart: React.FC<ExpensesChartProps> = ({ height, maxExpensesCount }) => {
  const { data: expensesData, isLoading, isError } = useExpensesData();

  if (isLoading) return <p>Loading...</p>;
  if (isError) return <p>Error fetching expenses</p>;

  if (!expensesData || expensesData.length === 0) return <p>No expenses data available</p>;

  const data = [...expensesData].sort((a, b) => a.cost - b.cost); 

  const displayedData = maxExpensesCount ? data.slice(0, maxExpensesCount) : data;
