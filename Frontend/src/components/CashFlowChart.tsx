...
const CashFlowChart = () => {
  ...
  const { data: monthlyData, isLoading: isMonthlyLoading, error: monthlyError } = useMonthlyData();
  const { data: yearlyData, isLoading: isYearlyLoading, error: yearlyError } = useYearlyData();
    ...
    <>
      ...
      <ResponsiveContainer width="100%" height={300}>
        <AreaChart data={data} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
          ...
          <XAxis stroke="#2A4F77" dataKey={dataKey} tick={{ fontSize: 12, fill: "#FFFFFF" }} />
          <YAxis stroke="#2A4F77" tick={{ fontSize: 12, fill: "#FFFFFF" }} />
          <Label value="Podział Zarobków i Wydatków" offset={0} position="top" fill="#FFFFFF" fontSize={16} fontWeight="bold" />
          <Legend />

          <Area type="monotone" dataKey="income" stroke="#F56C4E" fill="url(#colorIncome)" />
          <Area type="monotone" dataKey="expense" stroke="#4CAF50" strokeDasharray="5 5" fill="none" />
          <Tooltip
            contentStyle={{
